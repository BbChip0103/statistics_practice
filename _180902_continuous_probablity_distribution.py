...
if __name__=='__main__':
    ...
    def EXP_test():
        ...
        for i in range(1, 11):
            ...
            plt.bar(x_range, y_range, width=0.5)
    ...
    def CLT_test():
        ...
        for i in range(1, 10):
            plt.subplot(5, 2, i)
            # x_range = [x-(10*i * 0.5) for x in range(10*i)]
            # plt.plot(x_range, [f_Binom(x, 10*i, 0.2) for x in range(10*i)])
            plt.plot([f_Binom(x, 10*i, 0.2) for x in range(10*i)])
            plt.title('B({}, 0.5)'.format(10*i))
            print(sum([f_Binom(x, 10*i, 0.2) for x in range(10*i)]))
